...
class ObjActor:
    ...
    '''
    def __init__(self, x, y, name_object, animation_key, animation_speed = .5, state = None, creature = None, 
                    ai = None, container = None, item = None, equipment = None, stairs = None, exitportal = None):
...
class ObjGame:
    ...
    def transition_next(self):
        ...
        self.map_previous.append((PLAYER.x, PLAYER.y, self.current_map, self.current_rooms, self.current_objects))    
        ...
        if len(self.map_next) == 0:
            ...
            (PLAYER.x, PLAYER.y, self.current_map, self.current_rooms, self.current_objects) = self.map_next[-1]
    ...
    def transition_previous(self):
        ...
        if len(self.map_previous) != 0:
            ...
            self.map_next.append((PLAYER.x, PLAYER.y, self.current_map, self.current_rooms, self.current_objects))
            
            (PLAYER.x, PLAYER.y, self.current_map, self.current_rooms, self.current_objects) = self.map_previous[-1]
...
class CompExitPortal:
    ...
    def use(self):
        if self.owner.state == 'OPEN':
            ...
            draw_text(SURFACE_MAIN, "YOU WON!", constants.FONT_TITLE_SCREEN, screen_center, constants.COLOR_BLACK, center = True)
            
            pygame.display.update()
            
            filename = "data\winrecord_" + PLAYER.creature.name_instance + '.' + datetime.date.today().strftime('%d%B%Y') + (".txt")
...
def death_mouse(mouse):
    # print message alerting player that creature has died
    game_message(f"{mouse.creature.name_instance} is death! Eat him for more health!", constants.COLOR_GREEN)
...
def death_player(player):
    ...
    draw_text(SURFACE_MAIN, "YOU DIED!", constants.FONT_TITLE_SCREEN, screen_center, constants.COLOR_WHITE, center = True)
    
    pygame.display.update()
    
    filename = "data\lecacy_" + PLAYER.creature.name_instance + '.' + datetime.date.today().strftime('%d%B%Y') + (".txt")
...
def map_place_objects(room_list):
    ...
    current_level = len(GAME.map_previous) + 1
...
def cast_heal(caster, value):
    ...
    if caster.creature.current_hp == caster.creature.max_hp:
        game_message(caster.creature.name_instance + " the " + caster.name_object + " is already at full health!")
        return "canceled"
...
class UiButton:
    ...
    def draw(self):
        ...
        draw_text(self.surface, self.button_text, constants.FONT_DEBUG_MESSAGE, self.center_coords, self.c_c_text, center = True)
...
def menu_main():
    ...
    continue_game_button = UiButton(SURFACE_MAIN, 'CONTINUE', (150, 30), (title_x, continue_button_y))
    
    new_game_button = UiButton(SURFACE_MAIN, 'NEW GAME', (150, 30), (title_x, new_game_button_y))
    
    options_button = UiButton(SURFACE_MAIN, 'OPTIONS', (150, 30), (title_x, options_button_y))
    
    quit_button = UiButton(SURFACE_MAIN, 'QUIT', (150, 30), (title_x, quit_button_y))
    ...
    while menu_running:
        ...
        draw_text(SURFACE_MAIN, title_text, constants.FONT_TITLE_SCREEN, (title_x, title_y - 20), 
...
def menu_options():
    ...
    sound_effect_slider = UiSlider(SURFACE_MAIN, (125, 15),
                                (slider_x, sound_effect_slider_y),
                                constants.COLOR_RED,
                                constants.COLOR_GREEN,
                                PREFERENCES.vol_sound)
    
    music_effect_slider = UiSlider(SURFACE_MAIN, (125, 15),
                                (slider_x, music_effect_slider_y),
                                constants.COLOR_RED,
                                constants.COLOR_GREEN,
                                PREFERENCES.vol_music)
    
    save_button = UiButton(SURFACE_MAIN, 'SAVE', (60, 30), (slider_x, button_save_y), 
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if (event.button) == 1:
                    ...
                    mouse_line_selection <= len(print_list) - 1):                        
                        obj = PLAYER.container.inventory[mouse_line_selection].name_object
...
def gen_player(coords):
    ...
    creature_com = CompCreature("Greg", base_atk = 40, max_hp = 30, base_def = 40, death_function = death_player)
    PLAYER = ObjActor(x, y, "Python", 
                    animation_key = "A_PLAYER", animation_speed = 1,
...
def gen_stairs(coords, downwards = True):
    x, y = coords
    if downwards:
        stairs_com = CompStairs()
        stairs = ObjActor(x, y, "stairs", 
                        animation_key = "S_STAIRS_DOWN", 
                        stairs = stairs_com)
    else:
        stairs_com = CompStairs(downwards)
        stairs = ObjActor(x, y, "stairs",
                        animation_key = 'S_STAIRS_UP', 
                        stairs = stairs_com)
        
    GAME.current_objects.insert(-2, stairs)

def gen_portal(coords):
    x, y = coords
    portal_com = CompExitPortal()
    portal = ObjActor(x, y, 'exit portal', animation_key = 'S_PORTALCLOSED', exitportal = portal_com)
    
    GAME.current_objects.insert(-2, portal)
...
def gen_lamp(coords):
    ...
    return_object = ObjActor(x, y, 'THE LAMP', animation_key = 'S_MAGIC_LAMP', item = item_com)

    GAME.current_objects.insert(0, return_object)
...
def gen_scroll_lightning(coords):
    ...
    return_object = ObjActor(x, y, "Lightning scroll", animation_key = "S_SCROLL_01", item = item_com)
...
def gen_scroll_fireball_box(coords):
    ...
    return_object = ObjActor(x, y, "Fireball-box scroll", animation_key = "S_SCROLL_02", item = item_com)
...
def gen_scroll_fireball_diamond(coords):
    ...
    return_object = ObjActor(x, y, "Fireball-diamond scroll", animation_key = "S_SCROLL_04", item = item_com)
...
def gen_scroll_confusion(coords):
    ...
    return_object = ObjActor(x, y, "Confusion scroll", animation_key = "S_SCROLL_03", item = item_com)
...
def gen_weapon_sword(coords):
    ...
    return_object = ObjActor(x, y, "Sword", animation_key = "S_SWORD", equipment = equipment_com)
...
def gen_weapon_scythe(coords):
    ...
    return_object = ObjActor(x, y, "Scythe", animation_key = "S_LONG_SWORD", equipment = equipment_com)
...
def gen_weapon_dagger(coords):
    ...
    return_object = ObjActor(x, y, "Dagger", animation_key = "S_DAGGER", equipment = equipment_com)
...
def gen_armor_shield_diamond(coords):
    ...
    return_object = ObjActor(x, y, "Diamond shield", animation_key = "S_STRONG_SHIELD", equipment = equipment_com)
...
def gen_armor_shield(coords):
    ...
    return_object = ObjActor(x, y, "Shield", animation_key = "S_SHIELD", equipment = equipment_com)
...
def gen_armor_body(coords):
    ...
    return_object = ObjActor(x, y, "Armor", animation_key = "S_ARMOR", equipment = equipment_com)
...
def gen_armor_body_diamond(coords):
    ...
    return_object = ObjActor(x, y, "Diamond armor", animation_key = "S_STRONG_ARMOR", equipment = equipment_com)
...
def gen_mouse(coords):
    ...
    creature_com = CompCreature(creature_name,death_function = death_mouse, 
                                base_atk = base_attack, max_hp = max_health)
    
    ai_com = AiFlee()
    
    item_com = CompItem(use_function = cast_heal, value = 3)
    
    mouse = ObjActor(x, y, "mouse", 
...
def game_initialize():
    ...
    global SURFACE_MAIN, SURFACE_MAP, CLOCK, FOV_CALCULATE, ASSETS, CAMERA, RANDOM_ENGINE, PREFERENCES
...
def game_new():
    ...
    GAME = ObjGame()  # GAME tracks game progress
    gen_player((0, 0))
